    ...
    <div>
      <Container>
        <div className="grid grid-cols-1 gap-y-8 gap-x-4 sm:grid-cols-2 sm:gap-y-12">
          {casestudies
            .sort((caseStudyA, caseStudyB) => {
              const caseStudyADate = parse(
                caseStudyA.data.date,
                'yyyy-MM-dd',
                new Date()
              );
              const caseStudyBDate = parse(
                caseStudyB.data.date,
                'yyyy-MM-dd',
                new Date()
              );

              return caseStudyADate > caseStudyBDate ? -1 : 1;
            })
            .map((casestudy, index) => (
              <div
                key={index}
